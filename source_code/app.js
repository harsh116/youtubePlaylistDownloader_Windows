...
const mainDownload=async(playListName,downURL,title,type)=>{
  let extension=""
  if(type==='video')
    extension=".mp4" 
  else
...
{
  console.log('Error in accesing playlist. Make sure to have your playlist visibility is unlisted or public')
  return
}
...
for(let lis of list)
{
   ...
   mainDownload(playListName,downURL,title,"video")
...
const downloadAudio=async(url)=>
{
  ...
  for(let lis of list)
  {
     ...
     mainDownload(playListName,downURL,title,"audio")
...
if(str3.length>0)
{
  ...
  for(let lis of list)
  {
    if(lis.length==0||!(lis.length>4&&lis.substr(0,4).toLowerCase()==='http'))
    {
...
if(str1.length==0)
{
  console.log('Please start typing playlist links at playlist/playlist.txt separated by space or new line')
}
else{
